...
- **Hospital Registry & Bed Statistics** (SPKC STAC060 utilised bed capacity + STAC050/STAC070 bed profile by NUTS-3 region; SPKC AMB031 annual outpatient visits (doctor + nurse/midwife) by NUTS-3 region; CSP IRD041 NUTS-3 population; per-facility bed counts curated from operator sites, media coverage, and Wikipedia LV cross-checked against national bed totals) — [SPKC PxWeb](https://statistika.spkc.gov.lv/) · [CSP IRD041](https://data.stat.gov.lv/api/v1/en/OSP_PUB/START/POP/IR/IRD/IRD041)
- **Military Installations** (NBS _Nacionālie bruņotie spēki_ garrison roster — active-duty personnel curated from official unit references, Ministry of Defence sources, and NATO StratCom CoE publications) — [mil.lv](https://www.mil.lv/) · [mod.gov.lv](https://www.mod.gov.lv/)
...
- **Measured commute totals rather than inferred ones.** Slovakia publishes its 2021 census journey-to-work data as a full municipality-to-municipality matrix, so the totals travelling between each pair of municipalities are observed rather than estimated from the commuting behaviour of peer countries — the position countries without such a matrix are left in. Placement within those totals follows a gravity model with distance-decay.
...
- **Hospital demand.** Latvian hospitals have now been added as demand nodes. A curated 34-facility roster (acute, specialized, long-term, and psychiatric hospitals) is anchored to SPKC (_Slimību profilakses un kontroles centrs_) per-facility bed counts, with NUTS-3-regional bed occupancy from SPKC STAC060 and outpatient volumes from SPKC AMB031 (2024 vintages), and NUTS-3 populations from CSP IRD041. Because AMB031 publishes total ambulatory visits (GP + specialist + hospital-outpatient) rather than the hospital-outpatient-only slice used by CZ ÚZIS and PL BDL, a haircut is applied so LV per-bundle hospital demand lands in the same 2-3% peer band as its peers.

- **Military installation demand.** A hand-curated roster of 29 Nacionālie bruņotie spēki (_NBS_) / Zemessardze / NATO installations are now added as demand nodes — including the Ādaži cluster , the Liepāja Naval Base, Lielvārde Air Base, Aizsardzības ministrija and NBS HQ in Rīga, etc.
  - Given LV's cadastre does not distinguish military buildings from those of other types, base perimeters are extracted from OpenStreetMap polygons, and buildings inside active bases are zeroed from the workplace pipeline to prevent double-counting.

#### Bugfixes

- **Corrected Saulkrasti workplace attribution.** The Saulkrasti densely populated area previously had all of its ~2,000 workers concentrated on a single anchor point because the DPA polygon straddles two adjacent pagasti at essentially equal overlap and its centroid (and worker mass) drifted into the wrong one; the workforce is now correctly distributed across the actual city building fabric.

- **Regenerated Liepāja coastal water.** The Liepāja map's small oceanic slivers along the coast have been addressed and removed.
...
- **Hospitals**
  - Daily commute demand at inpatient and outpatient facilities across the 34-hospital LV roster (acute, specialized, long-term, and psychiatric). Per-facility bed counts are curated from operator sites and cross-checked against national bed totals; NUTS-3-regional occupancy comes from SPKC STAC060 and outpatient volumes from SPKC AMB031 (2024). Because AMB031 publishes total ambulatory (GP + specialist + hospital-outpatient) rather than the hospital-outpatient slice used by CZ / PL, a peer-grounded 0.41 hospital-share haircut is applied so LV per-bundle demand sits in the same 2-3% peer band. Psychiatric hospitals are classified as `long_term` so they draw only from bed occupancy and are excluded from the ambulatory outpatient denominator.
- **Military bases**
  - Active-duty personnel at 29 Nacionālie bruņotie spēki (_NBS_) / Zemessardze / NATO installations — Ādaži cluster (Mechanized Infantry Brigade HQ + subordinate battalions + Canadian-led NATO Multinational Brigade Latvia), Liepāja Naval Base, Lielvārde Air Base, MoD and NBS HQ in Rīga, the Zemessardze regional battalions, and the NATO StratCom Centre of Excellence.
